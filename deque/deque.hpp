...
namespace MySTL {
    ...
    class deque {
        ...
        class iterator{
            ...
            // here are some functions used fro debugging
            inline size_type get_d_Bufsize() const{
                return d->Bufsize;
            }

            inline size_type get_d_map_size() const{
                return d->map_size;
            }
        ...
        inline void move(deque& other){
            ...
            first=iterator{this,other.first.index,size_type(other.first.cur-other.first.first)};
            finish=iterator{this,other.finish.index,size_type(other.finish.cur-other.finish.first)};
            Bufsize=other.Bufsize;
        ...
        inline void move_to(deque& other){
            ...
            other.first=iterator{&other,first.index,size_type(first.cur-first.first)};
            other.finish=iterator{&other,first.index,size_type(finish.cur-finish.first)};
            map=nullptr;
        ...
        inline void reallocate_map(){
            ...
            delete[] tmp;
            capacity=Bufsize*map_size;
            first.d=finish.d=this;
            first.Bufsize=finish.Bufsize=Bufsize;
        ...
        inline size_type _deque_buf_size(size_type n,size_type sz) const{
            ...
            // return std::max(n,((sz>512)?size_type(1):size_type(512/sz)));
        }
        ...
        inline iterator insert_aux(iterator pos,const value_type& val,size_type n){
            ...
            if(elems_before<(currentSize>>1)){
                ...
                iterator twm=first+elems_before;
        ...
        deque(size_type n,const value_type& val=value_type{},allocator_type my_allocator=std::allocator<T>{}):Bufsize{_deque_buf_size(BufferSize,size_type(sizeof(val)))},
                                                                                map_size{std::max(n/Bufsize+1,INITIAL_SIZE)},
        ...
        deque(const std::initializer_list<value_type>& lst):
        Bufsize{_deque_buf_size(BufferSize,sizeof(value_type))}, map_size{std::max(lst.size()/Bufsize+1,INITIAL_SIZE)},
        ...
        deque(const std::initializer_list<value_type>& lst,allocator_type my_allocator):
                                                    Bufsize{_deque_buf_size(BufferSize,size_type(sizeof(value_type)))},
                                                    map_size{std::max(lst.size()/Bufsize+1,INITIAL_SIZE)},
        ...
        inline iterator erase(iterator _first,iterator _last){
            //注意last位置的元素不删除
            difference_type diff_index=_last-_first+1,_last_index=_last-first+1;
            currentSize-=(diff_index-1);
            ...
            return first+(_last_index-diff_index);
        }
